...
Trigger:
    idv_control.json landing under incoming/ is now the sole trigger
    (replaces the old trigger.done marker). Its content is inspected before
...
# Config
# ---------------------------------------------------------------------------
BUCKET_NAME = "aikycs3-raw-documents-dev"
...
# The control file is now the sole trigger AND the source of batch metadata
# used for validation. Replaces the old trigger.done marker entirely.
CONTROL_FILENAME = "20260716112152.json"
...
# ---------------------------------------------------------------------------
def _extract_lines(file_key: str, file_type: str) -> list[tuple[str, float]]:
...
def _load_control_file(bucket: str, key: str) -> Optional[dict]:
    """Fetch and parse idv_control.json. Returns None on any read/parse error."""
    try:
...
def _list_incoming_files(bucket: str, prefix: str) -> list[dict]:
...
# ---------------------------------------------------------------------------
def _validate_batch(control_data: dict, actual_files: list[dict]) -> list[dict]:
...
# ---------------------------------------------------------------------------
def _process_single_file(file_key: str, parsed: dict) -> dict:
    ...
    try:
        lines = _extract_lines(file_key, file_type)
...
# ---------------------------------------------------------------------------
# Batch processing - triggered once by idv_control.json (batch_complete=true)
# ---------------------------------------------------------------------------
def _process_batch(bucket: str, prefix: str, control_data: dict) -> list[dict]:
    ...
    raw_files = _list_incoming_files(bucket, prefix)
    print("Dedup")
    deduped_files, dedup_discrepancies = _dedup_latest_versions(raw_files)
 
    print("Validate discrepancies")
    validation_discrepancies = _validate_batch(control_data, raw_files)
    ...
    for parsed in deduped_files:
        ...
        results.append(_process_single_file(moved_key, parsed))
 
    # Relocate the control file itself too, for audit trail.
    control_key = f"{prefix}{CONTROL_FILENAME}"
    try:
...
def lambda_handler(event, context):  # noqa: ARG001  pylint: disable=unused-argument
    ...
    for record in event.get("Records", []):
        ...
        if file_name != CONTROL_FILENAME:
            logger.info(
                "Skipping %s - this lambda only triggers when %s is uploaded; "
                "individual document uploads are ignored until the control file arrives.",
                key, CONTROL_FILENAME,
            )
            continue
        ...
        results.extend(_process_batch(bucket, INCOMING_PREFIX, control_data))
