...
class ServiceLinkTransaction(BaseModel):
    ...
    agmt_id: str  # AGMT_ID
    account_no: str  # EAC_NO
    nsc: str  # EBRN_NSC
    transaction_date: Optional[date] = None  # ETX_DTE
    src: Optional[str] = None  # ESRCE_CD
    tx_narrative: Optional[str] = None  # ETX_NARR
    debit_eur: Optional[float] = None  # DEBIT_AMT
    credit_eur: Optional[float] = None  # CREDIT_AMT
    tx_code: Optional[str] = None  # ETX_CDE
    country_of_origin: Optional[str] = None  # Country of Origin

    @field_validator("debit_eur", "credit_eur")
    @classmethod
    def round_to_two_decimals(cls, v: Optional[float]) -> Optional[float]:
        """Round credit/debit always to 2 decimal places"""
        if v is not None:
            return round(v, 2)
        return v
